...
impl<'a> PeepholeOptimizations {
    ...
    pub fn remove_unused_class(
        ...
        match Self::classify_class_removability(c, &*ctx, ctx.scoping()) {
            ...
            // heritage, no present static values, pure computed keys — the
            // extracts-nothing `debug_assert` at the class removal site in
            // `remove_unused_declaration.rs` encodes exactly this — so the
            // extraction walk (which would re-run the classifier's purity
            // checks to extract nothing) is skipped.
