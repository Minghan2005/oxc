...
impl<'a> Compressor<'a> {
    ...
    fn run_in_loop(
        ...
        // traversal also doubled as the liveness seeding collection (see
        // `symbol_liveness::begin_seeding_pass`), so the same end-of-pass
        // sequence makes source-level dead cycles (`function f() { f() }`)
        // visible to pass 1 and seeds the edge filter — no standalone walk
        // anywhere. The continue signal is irrelevant pre-loop: the loop
        ...
        loop {
            ...
            // Convergence: another pass is demanded only for a dead symbol
            // or candidate absent from the previous set (both bounded by
            // the symbol table, and a quiet re-run collects an identical
            // set — see `propagate_collected`); the iteration cap backstops
            // pathological churn.
