...
///
/// This trait is a thin translation layer between untyped field pointers (managed by
/// [`MultiVec`], which performs all allocation, layout, and pointer arithmetic itself,
/// using [`SHAPE`]) and the typed field values / references of `Self`.
/// Its methods only cast the provided pointers to the field types and read / write /
/// drop / create references through them - they contain no logic of their own.
///
/// # The `Array` associated type
///
/// [`MultiVec`] handles one pointer (or offset) per field, always as a fixed-size array
/// `[T; N]`, where `N` is the number of fields. `N` is a property of the implementor
/// which the trait cannot name: `[T; Self::FIELD_COUNT]` in a trait signature is a
/// generic const expression, which stable Rust rejects wherever it appears (including
/// through a wrapper type like `Array<T, { Self::FIELD_COUNT }>`). Instead the array
/// *type* is an associated type constructor, [`Array<T>`], which every implementation
/// defines as `[T; N]` with its own concrete `N` - so the field count never appears as
/// a const generic parameter on this trait, [`MultiVec`], or the iterators.
///
/// [`Array<T>`] is bounded by the sealed `CopyArray<T>` trait (only `[T; N]` implements
/// it), which grants the operations [`MultiVec`] needs - indexing, copying - and
/// `CopyArray::from_fn`, since [`MultiVec`] cannot write an array literal of
/// statically unknown length.
///
/// Field types may need dropping (e.g. `String`): [`MultiVec`] drops the stored field values
/// (via [`drop_columns`]) when it is dropped. Note that elements only ever exist as
/// scattered field values, so `Self`'s own [`Drop`] impl (if it has one) is never invoked -
...
///   The `CopyArray<T>` bound guarantees each `Array<T>` is *some* `[T; N]`; that every
///   `T` uses the *same* `N` is on the implementor. On stable Rust this is statically
///   impossible to break, but on nightly it can be, with `#![feature(specialization)]`.
/// * The `[Layout; N]` passed to [`Shape::new`] in [`SHAPE`]'s initializer must be
///   the layout of each field's *type*, i.e. `Layout::new::<FieldType>()`, in the same
///   field order the methods interpret `ptrs`. It is not sufficient for each to merely be
///   a valid [`Layout`](std::alloc::Layout): [`MultiVec`] computes the address of element `index` in a field's
///   array as `field_array_start + index * field_size`, which relies on the guarantee
///   (which all Rust types have) that a type's size is a multiple of its alignment. An
///   artificial layout without that property would produce misaligned elements. This is
///   the only field-layout information [`MultiVec`] trusts - every derived quantity (offsets,
///   alignment, element size, capacity) is computed by `Shape::new`.
/// * Every method must interpret `ptrs[i]` as a pointer to the `i`th field's type (the
///   same field order as [`SHAPE`]'s layout list), and must do nothing beyond casting the
///   pointers and reading / writing / cloning / dropping / creating references or slices
///   through them. (This bullet also covers [`CloneFields`], whose implementations
///   must uphold these requirements too - for its `clone_columns`, "`ptrs[i]`" means
///   both of `ptrs[i]`'s pointers.)
///
/// [`MultiVec`]: super::MultiVec
...
pub unsafe trait Fields: Sized {
    // The view types are bounded `Self: 'v`.
    // Each borrows field data for `'v`, so `Self` must outlive `'v` -
    // a view can't outlast the data it points to.
    // (The slice views live on [`Slices`], for the same reason, plus its own.)
    ...
    ///
    /// See the trait docs for why this is an associated type. The `CopyArray<T>` bound
    /// (sealed - only `[T; N]` implements it) grants everything generic code does with the
    /// array: indexing to read the elements, copying it by value
    /// (which is also what lets [`Shape`], storing an `Array<usize>`, be [`Copy`]), and
    /// `CopyArray::from_fn` to construct one.
    ...
    ///
    /// This is the *only* layout information the implementor provides. The trusted input
    /// is the `[Layout; N]` passed to [`Shape::new`] (which must be the field types'
    /// layouts, in declaration order - see the safety contract). Every derived quantity
    /// is computed by `Shape::new`, so no generic code trusts an impl-supplied offset,
    /// size, or capacity.
    ///
    ...
    ///
    /// Does not read or drop any previous values at the locations: if a location holds an
    /// initialized value of a type which needs dropping, that value is leaked.
    ///
    /// # SAFETY
    ///
...
/// `IndexSlice<I, [_]>`. The [`Fields`] implementor is the *element* struct (e.g.
/// `Thing<'a>`), which does not carry the index type or its lifetimes, so a [`Fields`]
/// method could not name them. Taking `I` as a trait parameter lets the impl bind those
/// lifetimes - `unsafe impl<'k, 'a> Slices<ThingId<'k>> for Thing<'a>` - where the
/// [`Fields`] impl is only `impl<'a> ... for Thing<'a>`.
///
...
/// # SAFETY
///
/// Implementations must uphold [`Fields`]' requirements: interpret each `ptrs[i]` as a
/// pointer to the `i`th field's type (the same field order as [`SHAPE`]'s layout list),
/// and do nothing beyond casting the pointers and creating slices through them.
///
/// [`MultiVec`]: super::MultiVec
/// [`SHAPE`]: Fields::SHAPE
...
///
/// The `Clone` supertrait requires the fields struct to opt in too (derive `Clone` on
/// it as well). The struct's own `Clone` impl is never invoked (elements only ever exist
/// as scattered field values); for a derived (structural) `Clone` impl - the only sane
/// kind for a plain fields struct - field-wise and whole-struct cloning are equivalent.
///
/// # SAFETY
///
...
pub unsafe trait CloneFields: Fields + Clone {
    ...
    ///
    /// Fields with `Copy` types are copied bitwise (a single `memcpy` per array); fields
    /// with other `Clone` types are cloned element by element.
    ///
