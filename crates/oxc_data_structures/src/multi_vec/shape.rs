...
/// `Shape` cannot be forged or altered, even by the rest of this crate: every value
/// one holds is `new`'s output for *some* `[Layout; N]`, internally consistent by
/// construction. The only trusted input in the whole scheme is that layout list, which
/// [`Fields`]' safety contract requires to be the layouts of the actual field types
/// (the one thing only the implementor can know). Nothing generic reads impl-supplied
/// offsets, sizes, alignments, or capacities - they are all computed here.
///
...
impl<const N: usize> Shape<[usize; N]> {
    ...
    pub const fn new(field_layouts: [Layout; N]) -> Self {
        ...
        while i < N {
            ...
            // "Sorted before" is a strict total order over the fields, so every field has a distinct
            // `fields_before` - each lands in its own slot, and the array holds every field exactly once,
...
impl<A: CopyArray<usize>> Shape<A> {
    ...
    ///
    /// Guaranteed to be non-zero, and a power of two.
