...
///
/// This is a plain *description* - the numbers `(base_ptr, len, capacity)`, plus the
/// layout maths to locate any field of any element. It owns nothing (no `Drop` impl):
/// ownership of the elements and the allocation lies with the containing type
/// ([`MultiVec`], or the iterators in the `iter` module, which embed clones of a
/// `MultiVec`'s `Columns`). It is `Clone` but deliberately not `Copy`, so that every
/// duplication site is an explicit `.clone()` call.
///
/// `Columns` values are constructed by [`empty`], and otherwise built / mutated only by
/// `MultiVec` (in the `vec` module), which is responsible for upholding the invariants.
/// The `iter` module holds clones, and only reads them.
///
/// # Allocation layout
///
/// All columns live in a single allocation, sorted in order of descending alignment
/// (ties broken by declaration order). Sorting by alignment means no padding is ever
/// required between the columns: every column earlier in the allocation has element size
/// which is a multiple of its own alignment (a guarantee all Rust types have), which is in
/// turn a multiple of every later column's (smaller or equal, power of 2) alignment.
///
...
/// ```
///
/// With no padding, each column starts at `capacity * field_offsets[i]`. All the layout
/// quantities used here - `field_offsets`, the total `element_size`, the alignment
/// (`align`), and `max_alloc_capacity` below - are compile-time constants, precomputed from
/// the field types' layouts and read from [`F::SHAPE`]. The allocation's total size is
/// `capacity * element_size`, and its alignment is the maximum field alignment (`align`),
/// which the first column is aligned to.
///
...
/// ```
///
/// Overflow checks happen once, at allocation time: `capacity` never exceeds
/// `max_alloc_capacity`, which guarantees `capacity * element_size <= isize::MAX`, and
/// every offset the hot paths compute is at most `capacity * element_size`.
///
/// # Invariants
///
...
// regardless of `F`. But it is deliberately `Clone` and not `Copy` - each duplicate is
// an independent claim on the same allocation, so duplication should be explicit
// (a `.clone()` call at the site), never implicit.
...
impl<F: Fields> Columns<F> {
    ...
    ///
    /// Empty `Columns` (`len == capacity == 0`), with a dangling `base_ptr` aligned to the
    /// allocation's alignment. Trivially establishes the invariants.
    ...
    ///
    /// IMPORTANT: `Columns` does not own the allocation, and it will not be freed when `Columns`
    /// is dropped. The caller is responsible for freeing the allocation by calling [`deallocate`].
    ///
    /// # SAFETY
    ///
    /// Caller must ensure:
    ...
    pub unsafe fn allocate(capacity: usize) -> Self {
        ...
        // SAFETY: `capacity <= max_alloc_capacity` per this function's safety contract
        let layout = unsafe { Self::layout_for(capacity) };

        // SAFETY: `layout` has non-zero size: `capacity > 0` per this function's safety contract,
        // and at least one field has non-zero size (`Shape::new` rejects all-ZST field sets
        // at compile time).
    ...
    /// Free the allocation.
    ///
    /// Does not drop any elements - the caller is responsible for dropping them before this call.
    ///
    ...
    pub unsafe fn deallocate(&self) {
        ...
        // SAFETY: By `self`'s invariants (`capacity > 0`), `base_ptr` points to a live
        // allocation created by `allocate(capacity)`, so with layout
        // `Self::layout_for(capacity)` = `self.layout()`, which is deterministic - this
        // is the same layout it was allocated with.
        unsafe { alloc::dealloc(self.base_ptr.as_ptr(), self.layout()) };
    }

    /// Get the [`Layout`] of the current allocation.
    fn layout(&self) -> Layout {
        // SAFETY: `self.capacity <= max_alloc_capacity` by `Columns`'s invariants
    ...
    unsafe fn layout_for(capacity: usize) -> Layout {
        ...
        // SAFETY: `align` is a field type's alignment, so is a non-zero power of 2.
    ...
    pub fn slice_ptrs(&self) -> F::Array<NonNull<u8>> {
        ...
        // SAFETY: By `Column`'s invariants, `base_ptr` points to an allocation with layout
        // `self.layout()` (or is dangling and aligned to `align`, if `capacity == 0`).
        ...
        // `capacity * element_size` = the allocation's size, which is
        // `<= isize::MAX` when the allocation was created.
        ...
        // * Every field sorted before this one has alignment >= this field's, and size
        //   which is a multiple of its own alignment (per `Fields`' safety contract),
        //   hence a multiple of this field's (smaller or equal, power of 2) alignment.
        //   So `field_offsets[i]` (their sizes' sum), and therefore
        //   `capacity * field_offsets[i]`, is a multiple of this field's alignment.
    ...
    pub unsafe fn field_ptrs(&self, index: usize) -> F::Array<NonNull<u8>> {
        ...
        // SAFETY: The column's start plus `index` elements stays within the column, or one
        // past its end (`index == capacity`, allowed by this function's safety contract),
        // which is still within the allocation or one past its end.
    ...
    ///
    /// The copies are untyped (bytewise), which [`ptr::copy_nonoverlapping`] permits even
    /// for types containing padding: uninitialized (padding) bytes are propagated to the
    /// destination, where they land at the same positions - so destination elements are
    /// exactly as initialized as source elements.
    ///
    /// This is a *move* of the elements, valid for any field type: bitwise copies are only
    /// a valid *clone* for `Copy` types, so after the copy, the source elements must never
    /// be used again (not read, and not dropped).
    ///
    /// # SAFETY
    ///
    ...
    pub unsafe fn copy_fields(&self, dst_ptr: NonNull<u8>, dst_capacity: usize) {
        ...
        //
        // With few fields, the loop fully unrolls.
        //
        // With many fields (LLVM stops fully unrolling somewhere between 20 and 32),
        // the loop reads the pre-sorted table from constant data, with no bounds checks,
        // and no data-dependent loads.
        //
        ...
        // the whole table gets `memcpy`d to the stack (512 bytes for 32 fields).
        ...
        for i in 0..<F::Array<usize>>::LEN {
            ...
            // SAFETY: By `self`'s invariants, `self.base_ptr` points to an allocation with
            // layout `self.layout()`, and caller guarantees the first `len` elements of
            // every column are initialized (if `capacity == 0`, `base_ptr` is dangling but
            // aligned, and `len == 0`, so `bytes == 0`, for which a dangling aligned
            // pointer is valid).
