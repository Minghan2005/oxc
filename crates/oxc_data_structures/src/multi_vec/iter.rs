...
/// yielding each element's field pointers ([`F::Array<NonNull<u8>>`](Fields::Array)).
///
/// # Invariants
///
...
impl<F: Fields> RawIter<F> {
    ...
    fn remaining(&self) -> usize {
        // Cannot underflow. `index <= columns.len` by `RawIter`'s invariants.
...
impl<F: Fields> Iterator for RawIter<F> {
    ...
    ///
    /// By `self`'s invariants, the returned pointers are aligned and point to initialized
    /// values of the field types. What may be done through them is determined by the
    /// wrapping iterator (see `RawIter`'s invariants).
    #[inline]
...
// `next` reads the data via `create_ref` on that thread, while the origin thread can
// still access it through its own `&MultiVec` (shared borrows coexist with the iterator's).
...
// are `Send` (see `MultiVec`'s `Send` / `Sync` impls). Through a mutable reference,
// values can be moved out (e.g. `mem::replace`), so sending the iterator can move the
// stored data itself across threads - which is `F: Send` on the data, not a property of
// the `F::Mut<'_>` item type. See `Iter`'s `Send` impl for why the bound is on the data,
// not on the item type.
unsafe impl<F: Fields + Send> Send for IterMut<'_, F> {}

// SAFETY: A shared `&IterMut` exposes no access to the values (only `size_hint`), but
// `Sync` also makes `&IterMut: Send`, which allows shared access to `F` values from
// another thread if `Iterator::Item` references escape - the same conditions as
// `&mut MultiVec: Sync`: when the field types are `Sync`.
...
impl<F: Fields> Iterator for IntoIter<F> {
    ...
    fn next(&mut self) -> Option<F> {
        ...
        // `IntoIter::new`). The values are moved out, and never used again:
        // `RawIter::next` advanced past them, so they are neither yielded again nor
        // dropped by `IntoIter`'s `Drop` (which frees the allocation without reading them).
...
impl<F: Fields> Drop for IntoIter<F> {
    fn drop(&mut self) {
        ...
        // The `remaining > 0` check is an optimization: the common case is a fully
        // consumed iterator, where the check avoids computing the field pointers and
        // making a call to `drop_in_place` with a zero-length slice for every column
        // whose type needs dropping (slice drop glue is not always inlined).
        //
        ...
        if remaining > 0 {
            // SAFETY: `index <= columns.len <= capacity` by `RawIter`'s invariants, as
            // `field_ptrs` requires. It returns aligned pointers to element `index`'s
            // field values, and elements `index..len` (the `remaining` elements starting
            // there) are initialized, and valid for reads and writes - and owned by this
            // `IntoIter` (ownership was transferred to it by `IntoIter::new`).
